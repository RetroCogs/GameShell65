...
const argv = yargs(hideBin(process.argv))
        ...
        .option('poffs', {
            alias: 'z',
            description: 'if provided this will offset the colors in FCM',
            type: 'string',
        })  
        .option('nofill', {
            alias: 'l',
...
function getNCMDataForSprites(png, palette, wid, hei) {
    ...
    // Sprite sheet export 
    console.log("Exporting " + (png.width / (16 * wid)) + " x " + (png.height / (8 * hei)) + " sprites")

    for(var spry = 0; spry < png.height; spry += (8 * hei))
    {
        for(var sprx = 0; sprx < png.width; sprx += (16 * wid)) 
        {
            let charCols = [0]
            ...
            for(var tx=0; tx<wid; tx++) 
            {
                ...
                for(var ty=0; ty<hei; ty++)
                {
                    for(var r=0; r<8; r++) 
                    {
                        for(var c=0; c<16; c+=2) 
                        {
                            let i = ( row * (spry + ty * 8 ) + (sprx + tx * 16) + r*row + c)//row * (y + ty * 8 + r) + (x + tx * 16 + c)
...
//        charData = sortNCMDataForSprites(png, spriteWidth/16, spriteHeight/8,  paletteData.palette, charData, false)
    ...
    // fs.writeFileSync(path.resolve(outputPath, inputName+"_met.bin"), Buffer.from(metaBuffer))
